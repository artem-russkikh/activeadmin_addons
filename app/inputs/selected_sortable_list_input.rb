class SelectedSortableListInput < ActiveAdminAddons::InputBase
  ...
  def render_control_wrapper
    template.content_tag(:div, class: "selected-sortable-list-container") do
  ...
  def render_item(item)
    template.content_tag(:div, id: method_to_input_id(item.id), class: "selected-sortable-item") do
      template.concat(
        template.content_tag(:div, class: 'remove') { template.raw('&#x2716;') }
      )
